...
public class ActitimeLoginLogout2 {
	public static void main(String[] args) throws Throwable {
		...
		driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(5)); //implicitlyWait can be call directly
		
		
		WebDriverWait wait = new WebDriverWait(driver, Duration.ofSeconds(2));//for explicit wait we are WebDriverWait ref.
						...
						if (wait.until(ExpectedConditions.visibilityOf(logoutOption)).isDisplayed()) {
							...
							if (wait.until(ExpectedConditions.titleIs(expectedLoginPageTitle))) {
								...
								if (wait.until(ExpectedConditions.urlContains(expectedLoginPageUrl))) {
									...
									System.out.println("LoginPage is displayed after logging Out from the application.");
