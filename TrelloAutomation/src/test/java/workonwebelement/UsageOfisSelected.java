...
public class UsageOfisSelected {
	public static void main(String[] args) throws InterruptedException {
		...
		if (maleRadioButton.isSelected()) {
			...
			//before we select it is has not selected upon that confirmation we are moving ahead to select
			System.out.println("Fail");
			Thread.sleep(5000);
			//select the male radio button since it has not been selected 
			maleRadioButton.click();
			//verify that the male radio button has been selected after the selection or not
